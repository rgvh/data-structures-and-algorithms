...
const firstLetters = (arr) => {
  let firstLetterArray = arr.map(function(letterOne) {
    return letterOne.charAt(0);
  })
...
const findHappiness = (arr) => {
  let findSmileArray = arr.map(function(smileEmoji) {
    smileEmoji.filter(':)');
  })
  // console.log(findSmileArray);
  return findSmileArray;
};
