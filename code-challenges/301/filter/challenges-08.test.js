...
const oddValues = (arr) => {
  let newArray = arr.filter(n => (n % 2));
  return newArray;
};

//ALTERNATE
// const oddValuesWithIndex = (arr) => {
...
let someFilteredArray = arr.filter((value) => {
  if (value === 'hello') {
    return value;
  }
})

// console.log(someFilteredArray); //Output would be ['hello']

const filterStringsWithVowels = (arrOfWords) => {
  // Solution code here...
  let wordsWithVowels = arrOfWords.filter((word) => {
...
const getStatName = (arr, minBaseStat) => {
  let statFiltered = arr.filter(value => value.baseStat > minBaseStat);

  console.log(statFiltered.stat.name);

  return statFiltered.name;
  // let statNameArray = [];

  // Object.values(statFiltered).forEach(value => {
  //   statNameArray.push(value);

  //   console.log(statNameArray);
  // })
